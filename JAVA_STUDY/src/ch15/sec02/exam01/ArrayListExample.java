...
public class ArrayListExample {
	...
	public static void main(String[] args) {
		//<Board> => 제네릭. 배열에 저장할 타입을 명시한다, 뒤에오는 건 타입을 생략해도 상관없다.
		...
		//특정 인덱스 번호의 객체 가져오기
		Board board = list.get(2); //인덱스번호 2인 board 객체를 리턴해주므로 Board 타입으로 받는다.
		System.out.println(board.getSubject() + " " + board.getContent() + " " + board.getWriter());
		
		//모든 객체를 하나씩 가져오기
		for(int i=0; i<list.size(); i++) {
			Board b = list.get(i); //인덱스번호 2인 board 객체를 리턴해주므로 Board 타입으로 받는다.
